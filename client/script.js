...
sayHelloButton.addEventListener("mouseout", resetBackgroundColor)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    let request =  axios.get(`http://localhost:3000/repeat/jesus i do know`)
    request.then(res => console.log(res.data))
    let repeatText =  document.getElementById("repeat-text")
    repeatTexttextContent = request
...
const ayFunction = () => {
    axios.get(`http://localhost:3000/query-test?byebyebye&ohlalal`)
    .then(res => {
...
const createFood = (event) => {
    ...
    let foodInput = document.querySelector('#form-input')
    const body = {
